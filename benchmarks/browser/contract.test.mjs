...
function fixture() {
  return { version: 1, suite: "table-browser-reference-v1", sampleCount: 3, versions: options.pins,
    results: ["client", "window"].flatMap((scope) => ["mount", "filter", "sort", "page"].flatMap((workload) => ["tables", "ag-grid", "mui"].map((provider) => ({
      scope, workload, provider, size: 1000,
      readyMedianMs: 1, readySamplesMs: [.5, 1, 1.5],
      medianMs: 2, samplesMs: [1, 2, 3], checksum: 123, maxMountedRows: 18,
    })))) };
}
...
test("nonfinite samples, incorrect medians and oversized DOM fail", () => {
  for (const patch of [
    ...
    { samplesMs: [1, 0, 3] },
    { medianMs: 1 },
    { readySamplesMs: [.5, NaN, 1.5] },
    { readySamplesMs: [.5, 0, 1.5] },
    { readyMedianMs: .5 },
    { readySamplesMs: [1.5, 1, 1.5] },
    { maxMountedRows: 103 },
    { maxMountedRows: 1 },
  ]) {

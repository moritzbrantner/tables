...
Sessions retain O(matching rows) source-index memory deliberately. They do not eliminate full preparation costs or turn one-off deep windows into O(page-size) queries. See `docs/performance.md` for lifecycle, React Strict Mode, compatibility and ownership constraints. The AG Grid/MUI client-scope matrix now uses a prepared Tables session across page-only changes, matching the reference grids' retained filter/sort row-model lifetime. Filter/sort changes still prepare a new session inside the measured interaction; supplied-window scope keeps query work outside every provider's timer.

The browser-reference report records two latency phases from each exact invocation. `readySamplesMs` stops when the adapter has completed query work, React commit, and provider-specific asynchronous readiness. `samplesMs` continues through the existing two animation frames. The report contract checks both sample sets and their medians and requires every ready sample to precede its paired frame-complete sample. Relative timing gates continue to use the frame-complete metric; the ready phase is diagnostic evidence for sub-frame optimization, not a replacement headline latency or a pure CPU measurement.
